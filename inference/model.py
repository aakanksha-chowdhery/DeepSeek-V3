...
class MLA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], position_ids: Optional[torch.LongTensor] = None):
        ...
        if attn_impl == "naive":
            ...
            # Detach caches to avoid backward graph issues
            k_cache = self.k_cache.detach()
            v_cache = self.v_cache.detach()
            ...
            kv_cache = self.kv_cache.detach()
            pe_cache = self.pe_cache.detach()
